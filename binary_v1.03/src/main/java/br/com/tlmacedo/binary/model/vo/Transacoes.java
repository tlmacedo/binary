...
public class Transacoes implements Serializable {
    ...
    public Transacoes(Symbol symbol) {
        ...
        setContractTypeLastPriceProposal(CONTRACT_TYPE.valueOf(Operacao.getLastPriceProposal()[getSymbolId()].getValue().getContract_type().getDescricao().toUpperCase()));
        setLastBarrier(Operacao.getLastPriceProposal()[getSymbolId()].getValue().getBarrier());
    }
...
//                this.symbol = new SimpleObjectProperty<>(transaction.getSymbol());
        ...
        Operacao.getTransacoesObservableList().add(0, this);

    }

    public void newTransacao_SELL(Transaction transaction) {

//        Transacoes transacao = Operacao.getTransacoesObservableList().stream()
//                .filter(transacoes -> transacoes.getContract_id() == transaction.getContract_id())
//                .findFirst().get();

        int index = Operacao.getTransacoesObservableList().indexOf(this);
...
//            this.tickVenda = new SimpleObjectProperty<>(transacao.getTickVenda());
        ...
        Operacao.getTransacoesObservableList().set(index, this);

        Operacao.getTransacoesDAO().merger(this);
