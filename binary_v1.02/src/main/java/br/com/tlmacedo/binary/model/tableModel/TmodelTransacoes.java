...
public class TmodelTransacoes {
    ...
    public TmodelTransacoes(Integer symbol) {
        this.symbol = Operacoes.getSymbolObservableList().stream()
                .filter(symbol1 -> symbol1.idProperty().intValue() == symbol)
                .findFirst().get();
    }
...
//        getColId().setCellValueFactory(param -> param.getValue());
        ...
        getColDataHoraCompra().setCellValueFactory(cellData ->
                new SimpleStringProperty(LocalDateTime.ofInstant(Instant.ofEpochSecond(cellData.getValue().dataHoraCompraProperty().getValue()),
                        TimeZone.getDefault().toZoneId()).format(DateTimeFormatter.ofPattern("HH:mm:ss - dd/MM/yyyy"))));
        ...
        getColTickCompra().setCellValueFactory(cellData -> {
            if (cellData.getValue().tickCompraProperty().getValue() != null)
                return new SimpleStringProperty(ServiceMascara.getValorFormatado(getSymbol().getPip_size(), cellData.getValue().tickCompraProperty().getValue()));
        ...
        getColTickVenda().setCellValueFactory(cellData -> {
            if (cellData.getValue().tickVendaProperty().getValue() != null)
                return new SimpleStringProperty(ServiceMascara.getValorFormatado(getSymbol().getPip_size(), cellData.getValue().tickVendaProperty().getValue()));
            return new SimpleStringProperty("");
        });
        ...
        getColStakeCompra().setCellValueFactory(cellData -> {
            if (cellData.getValue().stakeCompraProperty().getValue() != null)
                return new SimpleStringProperty(ServiceMascara.getValorMoeda(cellData.getValue().stakeCompraProperty().getValue()
